...
public class ShellCatcherModuleMenu extends AbstractContainerMenu {
    ...
    public ShellCatcherModuleMenu(int id, Inventory playerInventory, Container container) {
        super(ModMenuTypes.SHELL_CATCHER_MODULE.get(), id);
        this.container = container;

        // Add slots for Shell Catcher Module inventory (27 slots)
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
    ...
    public ShellCatcherModuleMenu(int i, Inventory inventory, FriendlyByteBuf friendlyByteBuf) {
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        // Handle shift-clicking items between the container and player inventory
