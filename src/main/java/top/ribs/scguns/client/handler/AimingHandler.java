    ...
    public boolean isLookingAtInteractableBlock()
    {
        ...
        if(mc.hitResult != null && mc.level != null)
        {
            if(mc.hitResult instanceof BlockHitResult result)
            {
                ...
                return block instanceof EntityBlock || block == ModBlocks.GUN_BENCH .get()|| block == ModBlocks.MACERATOR .get()|| block == Blocks.CRAFTING_TABLE ||  state.is(BlockTags.DOORS) || state.is(BlockTags.TRAPDOORS) || state.is(Tags.Blocks.CHESTS) || state.is(Tags.Blocks.FENCE_GATES);
            }
